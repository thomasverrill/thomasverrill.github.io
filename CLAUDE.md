...
- **`things-to-do.astro` entry format**: each area (e.g. "ireland", "philadelphia") is its own `<section id=... aria-labelledby=...>` with an `.archive-list`, and areas are ordered alphabetically in both the `PageNav` items and the page body. An entry is an `<li>` containing a linked, lowercase title (`<a href="...">the entry title</a>`) followed by a `<p class="section-note">` with a 1–2 sentence description (per the lowercase and minimalist-copy conventions below). Adding a new area means adding both a `PageNav` item (`{ label: "area", href: "#area" }`) and a matching `<section>` block — don't reintroduce the generic `sections.map(...)` loop, since areas now carry distinct content. See the `ireland` section for the reference example.
